...
class WebsiteMembershipSignup(http.Controller):
    ...
    def website_membership_signup(self, membership_id, **kw):
        ...
        membership = request.env['product.template'].sudo().browse( int(membership_id) )
    ...
    def website_membership_signup_process(self, **kwargs):
        ...
        membership = request.env['product.template'].sudo().browse( int(values['membership_id']) )

        #Create the new user
        new_user = request.env['res.users'].sudo().create({'name': values['name'], 'login': values['email'], 'email': values['email'], 'password': values['password'],  'gender': values['gender']})
