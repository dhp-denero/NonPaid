...
class ir_ui_menu(models.Model):
    ...
    _defaults = {
        'parent_id': lambda self, cr, uid, ctx: ctx and ctx.get('default_iframe_pages_group') and self.pool.get('ir.model.data').get_object_reference(cr, uid, 'vuente_help_support', 'menu_top')[1]
    }
...
class page(models.Model):
    ...
    def get_default_menu_id(self):
        ...
        action = self.env['ir.actions.client'].create({
            'name': name,
            'tag': 'vuente_help_support.iframe',
        })
